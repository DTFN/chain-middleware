...
import (
	...
	"chainmaker.org/chainmaker/tcip-relayer/v2/module/handler"
...
func (s *RPCServer) Start() error {
	...
	go func() {
		if s.config.TLSConfig.Mode == tlsModeDisable {
			err = s.mixServer.ListenAndServe()
		} else {
...
func (s *RPCServer) RegisterHandler() error {
	...
	tcipApi.RegisterRpcRelayChainServer(s.grpcServer, apiHandler)
	return nil
}
...
func newGrpc(rpcConfig *conf.RpcConfig) (*grpc.Server, error) {
	...
	if rpcConfig.TLSConfig.Mode != tlsModeDisable {
		checkClientAuth := tls.NoClientCert
		if rpcConfig.TLSConfig.Mode == tlsModeToway {
...
func newMixServer(grpcServer *grpc.Server, rpcConfig *conf.RpcConfig) (*http.Server, error) {
	...
	if err := tcipApi.RegisterRpcRelayChainHandlerFromEndpoint(ctx, restMux, "localhost"+endPoint, dopts); err != nil {
