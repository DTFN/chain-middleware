	...
	case common.Version_V1_0_0:
		if req.CrossChainId == "$relayer" {
			...
			for _, chainRid := range chainRids {
				...
				paramData := []string{param.DID, param.DIDDocument}
				paramDataBytes, _ := json.Marshal(paramData)

				tryResult, tx, err := chain_client.ChainClientV1.InvokeContract(chainRid, didManagerAddress, "updateDID", DID_MANAGER_ABI, string(paramDataBytes), true, []tcipcommon.EventDataType{tcipcommon.EventDataType_STRING, tcipcommon.EventDataType_STRING})
			...
			// 动态构建ABI
			vc_busi_func_abi := fmt.Sprintf("[{\"constant\":false,\"inputs\":[{\"name\":\"vcs\",\"type\":\"string\"}],\"name\":\"%s\",\"outputs\":[],\"payable\":false,\"stateMutability\":\"nonpayable\",\"type\":\"function\"}]", req.CrossChainMsg.Method)

			// 编码参数
			paramData := []string{dataVc["data"].(string)}
			paramDataBytes, _ := json.Marshal(paramData)

			tryResult, tx, err := chain_client.ChainClientV1.InvokeContract(req.CrossChainMsg.ChainRid, req.CrossChainMsg.ContractName, req.CrossChainMsg.Method, vc_busi_func_abi, string(paramDataBytes), true, []tcipcommon.EventDataType{tcipcommon.EventDataType_STRING})
			if (err != nil) {
		...
		h.log.Debugf("[CrossChainTry] chainExist: %+v", chainExist)
		if !chainExist {
			h.log.Debugf("[CrossChainTry] ChainRid[%s] not exist", req.CrossChainMsg.ChainRid)
			h.log.Debugf("[CrossChainTry] req.CrossChainId: %s, req.CrossChainMsg.ExtraData: %s", req.CrossChainId, req.CrossChainMsg.ExtraData)
			{
...
func (h *Handler) ChainIdentity(ctx context.Context,
	...
	if req.BcosConfig == nil {
		return &cross_chain.ChainIdentityResponse{
			Code:    common.Code_INVALID_PARAMETER,
			Message: "BcosConfig is required",
		}, nil
	}
	switch req.Version {
	case common.Version_V1_0_0:
		switch req.Operate {
		case common.Operate_GET:
			bcosConfig, err := chain_config.ChainConfigManager.Get(req.BcosConfig.ChainRid)
			if err != nil {
				return &cross_chain.ChainIdentityResponse{
					Code:    common.Code_INTERNAL_ERROR,
					Message: err.Error(),
		...
		case common.Operate_SAVE:
			err := chain_config.ChainConfigManager.Save(req.BcosConfig, common.Operate_SAVE)
			if err != nil {
				return &cross_chain.ChainIdentityResponse{
					Code:    common.Code_INTERNAL_ERROR,
					Message: err.Error(),
		...
		case common.Operate_UPDATE:
			err := chain_config.ChainConfigManager.Save(req.BcosConfig, common.Operate_UPDATE)
			if err != nil {
				return &cross_chain.ChainIdentityResponse{
					Code:    common.Code_INTERNAL_ERROR,
					Message: err.Error(),
