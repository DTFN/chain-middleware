...
func (g *GrpcRequest) SyncBlockHeader(
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(timeout)*time.Second)
	defer cancel()
	response, err := client.SyncBlockHeader(ctx, req)
	_ = conn.Close()
	return response, err
}
...
//  @return error
func (g *GrpcRequest) InitSpvContract(req *relay_chain.InitContractRequest) (*relay_chain.InitContractResponse, error) {
	timeout := conf.Config.BaseConfig.DefaultTimeout
	client, conn, err := g.getConnection()
	if err != nil {
		return nil, err
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(timeout)*time.Second)
	defer cancel()
	response, err := client.InitContract(ctx, req)
	_ = conn.Close()
	return response, err
}
...
//  @return error
func (g *GrpcRequest) UpdateSpvContract(
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(timeout)*time.Second)
	defer cancel()
	response, err := client.UpdateContract(ctx, req)
	_ = conn.Close()
	return response, err
}
