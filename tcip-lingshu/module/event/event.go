...
func (e *EventManager) parseVcAndEncodeEvmParam(
	...
	// 虚拟abi
	// virtual_param := []map[string]interface{}{
	// 	{
	// 		"constant": false,
	...
	// virtual_abi, _ := json.Marshal(virtual_param)
	// contractAbi, _ := abi.NewAbi(string(virtual_abi))
	// inputData, _ := utils.Pack(contractAbi, method, string(parameterStr))
	// inputDataHexStr := hex.EncodeToString(inputData)
	// encodedParameter := fmt.Sprintf("{\"data\":\"%s\"}", dataVcStr)
