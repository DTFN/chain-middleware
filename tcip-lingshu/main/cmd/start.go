...
import (
	...
	"chainmaker.org/chainmaker/tcip-lingshu/v2/module/rpcserver"
	"chainmaker.org/chainmaker/tcip-lingshu/v2/module/server"
...
func StartCMD() *cobra.Command {
	startCmd := &cobra.Command{
		Use:   "start",
		Short: "Startup tcip-lingshu",
		Long:  "Startup tcip-lingshu",
		RunE: func(cmd *cobra.Command, _ []string) error {
			initLocalConfig(cmd)
			mainStart()
			fmt.Println("tcip-lingshu exit")
			return nil
		},
	}
...
func startFlagSet() *pflag.FlagSet {
	...
	flags.StringVarP(&conf.ConfigFilePath, flagNameOfConfigFilepath, flagNameShortHandOfConfigFilepath,
		conf.ConfigFilePath, "specify config file path, if not set, default use ./tcip_lingshu.yml")
	return flags
}
