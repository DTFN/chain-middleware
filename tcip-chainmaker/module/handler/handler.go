...
import (
	...
	chain_client "chainmaker.org/chainmaker/tcip-chainmaker/v2/module/chain-client"
...
func (h *Handler) CrossChainTry(ctx context.Context,
	...
	if req.CrossChainId == "$relayer" {
		...
		for _, chainRid := range chainRids {
			...
			// 编码参数
			// 参考 {"method":"minter","to":"1abf9c3c65beabd6270e94c0ccceb94ef4442f53bb85dccd526be4ad01d66b03","value":"10"}
			paramDataBytes, _ := json.Marshal(
				map[string]string{
					"did": param.DID,
					"didDoc": param.DIDDocument,
				},
			)

			h.log.Debugf("[CrossChainTry] inputDataHex: %s", string(paramDataBytes))
			tryResult, tx, err := chain_client.ChainClientV1.InvokeContract(chainRid, "DID_MANAGER", "updateDID", true, string(paramDataBytes), -1)
			h.log.Debugf("[CrossChainTry] didManager contract result: %+v, tx: %s, err: %+v", tryResult, tx, err)
		}
	...
	case common.Version_V1_0_0:
		...
		tryResult, tx, err := chain_client.ChainClientV1.InvokeContract(req.CrossChainMsg.ChainRid,
			req.CrossChainMsg.ContractName, req.CrossChainMsg.Method,
			true, req.CrossChainMsg.Parameter, -1)
		if err != nil {
...
func (h *Handler) CrossChainConfirm(ctx context.Context,
	...
	case common.Version_V1_0_0:
		...
		if req.CrossChainFlag == event.TbisFlag {
			paramByte, err1 := fillTbisResult(req.ConfirmInfo.Parameter, req.ConfirmInfo.ChainRid,
				event.SubSuccess, event.SubSuccess, req.TryResult[0])
			if err1 != nil {
			...
			param, err = fillTryResult(req.ConfirmInfo.Parameter, req.TryResult, req.CrossType)
			if err != nil {
				h.log.Errorf("[CrossChainConfirm] %s", err.Error())
				return &cross_chain.CrossChainConfirmResponse{
					Code:    common.Code_INTERNAL_ERROR,
					Message: err.Error(),
				}, nil
		...
		_, tx, err := chain_client.ChainClientV1.InvokeContract(req.ConfirmInfo.ChainRid,
			req.ConfirmInfo.ContractName, req.ConfirmInfo.Method, true,
			param, -1)
		if err != nil {
			h.log.Errorf("[CrossChainConfirm] Failed to execute cross-chain transaction: cross chain id: %s",
				req.CrossChainId)
			return &cross_chain.CrossChainConfirmResponse{
				Code:    common.Code_INTERNAL_ERROR,
				Message: err.Error(),
...
func (h *Handler) CrossChainCancel(
	...
	case common.Version_V1_0_0:
		...
		if req.CrossChainFlag == event.TbisFlag {
			paramByte, err := fillTbisResult(req.CancelInfo.Parameter, req.CancelInfo.ChainRid,
				event.SubFailed, event.SubFailed, "failed")
			if err != nil {
				h.log.Errorf("[CrossChainCancel] %s", err.Error())
				return &cross_chain.CrossChainCancelResponse{
					Code:    common.Code_INTERNAL_ERROR,
					Message: err.Error(),
				}, nil
		...
		_, tx, err := chain_client.ChainClientV1.InvokeContract(req.CancelInfo.ChainRid,
			...
			true, param, -1)
		if err != nil {
			h.log.Errorf("[CrossChainCancel] Failed to execute cross-chain transaction: cross chain id: %s",
				req.CrossChainId)
			return &cross_chain.CrossChainCancelResponse{
				Code:    common.Code_INTERNAL_ERROR,
				Message: err.Error(),
...
func (h *Handler) TxVerify(ctx context.Context,
	in *cross_chain.TxVerifyRequest) (*cross_chain.TxVerifyResponse, error) {
	return &cross_chain.TxVerifyResponse{
		TxVerifyResult: false,
		Code:           common.Code_TX_PROVE_ERROR,
		Message:        "tcip-chainmaker not support rpv tx_verify_type",
	}, nil
}
...
func (h *Handler) ChainIdentity(ctx context.Context,
	...
	if req.ChainmakerConfig == nil {
		return &cross_chain.ChainIdentityResponse{
			Code:    common.Code_INVALID_PARAMETER,
			Message: "ChainmakerConfig is required",
		}, nil
	}
	switch req.Version {
	case common.Version_V1_0_0:
		switch req.Operate {
		case common.Operate_GET:
			chainConfig, err := chain_config.ChainConfigManager.Get(req.ChainmakerConfig.ChainRid)
			if err != nil {
				return &cross_chain.ChainIdentityResponse{
					Code:    common.Code_INTERNAL_ERROR,
					Message: err.Error(),
			...
			return &cross_chain.ChainIdentityResponse{
				Code:             common.Code_GATEWAY_SUCCESS,
				Message:          common.Code_GATEWAY_SUCCESS.String(),
				ChainmakerConfig: chainConfig,
			}, nil
		case common.Operate_DELETE:
			err := chain_config.ChainConfigManager.Delete(req.ChainmakerConfig.ChainRid)
			if err != nil {
				return &cross_chain.ChainIdentityResponse{
					Code:    common.Code_INTERNAL_ERROR,
					Message: err.Error(),
		...
		case common.Operate_SAVE:
			err := chain_config.ChainConfigManager.Save(req.ChainmakerConfig, common.Operate_SAVE)
			if err != nil {
				return &cross_chain.ChainIdentityResponse{
					Code:    common.Code_INTERNAL_ERROR,
					Message: err.Error(),
		...
		case common.Operate_UPDATE:
			err := chain_config.ChainConfigManager.Save(req.ChainmakerConfig, common.Operate_UPDATE)
			if err != nil {
				return &cross_chain.ChainIdentityResponse{
					Code:    common.Code_INTERNAL_ERROR,
					Message: err.Error(),
...
// getCrossChainTryReturn 获取cross chain try的返回值
//  @param code
//  @param crossChainId
...
func fillTryResult(param string, tryResult []string, crossType common.CrossType) (string, error) {
	...
	tmpArr := make([]interface{}, len(tryResult))
	for i, v := range tryResult {
		tmpArr[i] = v
	}
	param = fmt.Sprintf(param, tmpArr...)
	return param, nil
}
...
//  @return []byte
//  @return error
func fillTbisResult(kvJsonStr, chainRid string,
	proveStatus, contractStatus int, contractResult string) ([]byte, error) {
	res := event.GetCommitParam(chainRid, proveStatus, contractStatus, contractResult)
	var kvMap map[string]string
	err := json.Unmarshal([]byte(kvJsonStr), &kvMap)
	if err != nil {
		err1 := fmt.Errorf("[fillTbisResult] kvJsonStr must be json string: %s -> %s",
			kvJsonStr, err.Error())
		return nil, err1
	}
	kvMap[event.SubResult] = res
