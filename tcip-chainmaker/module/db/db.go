...
func (d *DbHandle) Get(key []byte) ([]byte, error) {
	...
	if err != nil {
		msg := fmt.Sprintf("[Get] getting DbHandle key [%s], err:%s", string(key), err.Error())
		d.log.Errorf(msg)
		return nil, errors.New(msg)
	}
	d.log.Debugf("[Get] key: %s, value: %s", string(key), string(value))
	return value, nil
}
...
func (d *DbHandle) Put(key []byte, value []byte) error {
	msg := fmt.Sprintf("[Put] writing leveldbprovider key [%s] with nil value", string(key))
	if value == nil {
		d.log.Warn(msg)
...
func (d *DbHandle) Has(key []byte) (bool, error) {
	...
	if err != nil {
		d.log.Errorf("getting leveldbprovider key [%s], err:%s", string(key), err.Error())
		return false, fmt.Errorf("error getting leveldbprovider key [%s]", string(key))
	}
	return exist, nil
}
...
func (d *DbHandle) Delete(key []byte) error {
	...
	if err != nil {
		...
		return fmt.Errorf("error deleting leveldbprovider key [%s]", string(key))
	}
	return err
}
