...
Usage:
    ...
    -h Help
e.g:
    bash $0 -c channel1 -p ./NODES_ROOT/1ef243fd87 -m Org1MSP -n 192.168.3.128:7051 -s peer0.org1.example.com -g org1.example.com
EOF
exit 0
}
...
parse_params()
{
    while getopts "c:p:m:n:s:g:h" option;do
        case $option in
        c) channel_id="${OPTARG}"
...
main()
{
    ...
    peer channel join -b "${chain_path}/config/channel-artifacts/${channel_id}.block" > "${logfile}" 2>&1
    #rm "${logfile}"
}
...
{
    echo "=============================================================="
    LOG_INFO "Peer ${peer_name} join Channel ${channel_id} "
    LOG_INFO "All completed."
}

echo "=======$(date)========" >>"${logfile}"
echo "=======start join app channel===============" >>"${logfile}"
parse_params "$@"
main
print_result
echo "=======$(date)========" >>"${logfile}"
echo "=======end join app channel=================" >>"${logfile}"
