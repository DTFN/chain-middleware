...
main()
{
    ...
    peer channel fetch config "${chain_path}/config/channel-artifacts/config_block.pb" -o "${orderer_endpoint}" -c "${channel_id}" --tls --cafile "${chain_path}/organizations/ordererOrganizations/${orderer_org_name}/orderers/${orderer_name}/msp/tlscacerts/tlsca.${orderer_org_name}-cert.pem" > "${logfile}" 2>&1
    #rm "${logfile}"
}
...
{
    echo "=============================================================="
    LOG_INFO "Fetch Channel ${channel_id} config : ${chain_path}/config/channel-artifacts/config_block.pb"
    LOG_INFO "All completed."
}

echo "=======$(date)========" >>"${logfile}"
echo "=======start fetch app channel config===============" >>"${logfile}"
parse_params "$@"
main
print_result
echo "=======$(date)========" >>"${logfile}"
echo "=======end fetch app channel config=================" >>"${logfile}"
