...
main()
{
    ...
    peer channel create -o "${orderer_endpoint}" -c "${channel_id}" -f "${chain_path}/config/channel-artifacts/${channel_id}.tx" --outputBlock "${chain_path}/config/channel-artifacts/${channel_id}.block" --tls --cafile "${chain_path}/organizations/ordererOrganizations/${orderer_org_name}/orderers/${orderer_name}/msp/tlscacerts/tlsca.${orderer_org_name}-cert.pem" > "${logfile}" 2>&1
    #rm "${logfile}"
}
...
{
    echo "=============================================================="
    LOG_INFO "Channel Block Path   : ${chain_path}/config/channel-artifacts/${channel_id}.block"
    LOG_INFO "All completed."
}

echo "=======$(date)========" >>"${logfile}"
echo "=======start create app channel block===============" >>"${logfile}"
parse_params "$@"
main
print_result
echo "=======$(date)========" >>"${logfile}"
echo "=======end create app channel block=================" >>"${logfile}"
