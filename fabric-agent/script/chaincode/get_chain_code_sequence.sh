...
Usage:
    ...
    -h Help
e.g:
    bash $0 -c channel1 -n basic -m Org1MSP -e 192.168.56.102:7051 -s peer0.org1.example.com -g org1.example.com -f ~/fabric
EOF
exit 0
}
...
parse_params()
{
    while getopts "c:f:m:n:e:s:g:h" option;do
        case $option in
        c) channel_id="${OPTARG}"
            if [ -z "$channel_id" ]; then LOG_WARN "$channel_id not exist" && exit 1; fi
        ;;
...
main()
{
    ...
    log=$(peer lifecycle chaincode queryapproved -C "${channel_id}" -n "${chain_code_name}" 2>&1)
    echo "$log"
}
