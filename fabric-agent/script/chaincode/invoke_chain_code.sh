...
Usage:
    ...
    -a <ca file>                 [Required]
    -r <root cert file>          [Required]
    -o <order endpoint>          [Required]
    -w <function with args>      [Required]
    ...
    -h Help
e.g:
    bash $0 -t invoke -c channel1 -m Org1MSP -e 192.168.56.102:7051 -s peer0.org1.example.com -g org1.example.com -f ~/fabric -a orderer0.org1.example.com -r org1.example.com-o 192.168.56.102:7050 -n basic -w '{"function":"GetAllAssets","Args":[]}'
EOF
exit 0
}
...
parse_params()
{
    while getopts "t:c:p:a:r:o:w:n:v:f:m:e:s:g:h" option;do
        case $option in
        t) method="${OPTARG}"
            if [ -z "$method" ]; then LOG_WARN "$method not exist" && exit 1; fi
        ;;
        c) channel_id="${OPTARG}"
        ...
        o) orderer_endpoint="${OPTARG}"
            if [ -z "$orderer_endpoint" ]; then LOG_WARN "$orderer_endpoint not exist" && exit 1; fi
        ;;
        w) function_with_args="${OPTARG}"
            if [ -z "$function_with_args" ]; then LOG_WARN "$function_with_args not specified" && exit 1; fi
        ;;
...
main()
{
    ...
    log=$(peer chaincode "${method}" -o "${orderer_endpoint}" -C "${channel_id}" -n "${chain_code_name}" --peerAddresses "${peer_endpoint}" --tlsRootCertFiles "${chain_path}/organizations/peerOrganizations/${peer_org_name}/peers/${peer_name}/tls/ca.crt" --tls --cafile "${chain_path}/organizations/ordererOrganizations/${orderer_org_name}/orderers/${orderer_name}/msp/tlscacerts/tlsca.${orderer_org_name}-cert.pem"  -c "${function_with_args}" 2>&1)
    echo "$log"
}
