...
main()
{
    ...
    peer lifecycle chaincode approveformyorg -o "${orderer_endpoint}" --channelID "${channel_id}" --name "${chain_code_name}" --version "${chain_code_version}" --package-id "${package_id}" --sequence "${sequence}" --tls --cafile "${chain_path}/organizations/ordererOrganizations/${orderer_org_name}/orderers/${orderer_name}/msp/tlscacerts/tlsca.${orderer_org_name}-cert.pem"
