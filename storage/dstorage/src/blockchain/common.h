...
inline void compileSolidity(const std::string& contract_name, const std::string& file_name) {
    ...
    std::cout << "solc path is : " << command << std::endl;
    if (system(command.c_str()) != 0) {
        throw std::runtime_error("create abi and bin file had failed");
    }
}
...
inline std::string ToHex(const std::vector<uint8_t>& data, const std::string& prefix = "") {
    static const char* hexdigits = "0123456789abcdef";
    std::string hex;
    hex.reserve(prefix.size() + data.size() * 2);
