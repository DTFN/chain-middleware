...
bool BcosChain::Deploy(
    ...
    std::cout << "deploy " << contract_name << std::endl;
#ifdef USE_BCOS
    (void)contract_type;
    if (sdk_ == nullptr) {
        return false;
    ...
    std::string abi = ReadFile(abi_path);
    std::string bin = ReadFile(bin_path);
    ...
    if (params.size() == 1) {
        std::string output = "";
        std::string address = GetAddress(contract_name);
        Call(contract_type, contract_name, address, "init", params, output);
...
bool BcosChain::Call(const std::string contract_type, const std::string contract_name,
    ...
    std::string& output) {
    std::cout << "call " << contract_name << " " << contract_method << std::endl;
#ifdef USE_BCOS
    (void)contract_type;
    ...
    std::cout << "resp_data : " << resp_data << std::endl;
    // 找到最后一个 '}'
    ...
    if (d.HasMember("result") && d["result"].HasMember("output")) {
        ...
        if (contract_method == "getChallengeInfo") {
            ...
            if (d.HasMember("result") && d["result"].HasMember("status")) {
                int status = d["result"]["status"].GetInt();
            ...
            output = bcos_sdk_abi_decode_method_output(
                abi.c_str(), contract_method.c_str(), result_out.c_str(), sdk_crypto_type_);
            std::cout << "storeFile output " << output << std::endl;
        } else if (contract_method == "hasFile") {
            output = bcos_sdk_abi_decode_method_output(
                abi.c_str(), contract_method.c_str(), result_out.c_str(), sdk_crypto_type_);
            std::cout << "hasFile output " << output << std::endl;
        }
        // 这里调用 bcos_sdk_abi_decode_method_output(output)
    } else {
