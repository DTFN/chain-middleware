...
func invokeStoreFile(sdkPath string, abiPath string, contractName string, methodName string, fileId string, fileName string, fileSize C.longlong, totalShards C.longlong, dataShards C.longlong, ipfsUrls []string, cids []string) *C.char { 
	// Marshal 转 JSON
...
func invokeFileStoreGetMetaData(sdkPath string, abiPath string, contractName string, methodName string, fileId string) *C.char { 
	// 加载 abi
	abiJson, err := ioutil.ReadFile(abiPath) // 你合约编译生成的 ABI 文件
	if err != nil {
		return C.CString("")
	...
	dataByte, err := parsedABI.Pack(methodName, fileId)
	if err != nil {
		return C.CString("")
	}
