...
func main() {
	ret := invokeFabricStoreFile("FileStoreContract", "StoreFile", "cid123457", "sample.pdf", 123456, 4, 3, []string{"ipfs1", "ipfs2", "ipfs3", "ipfs4"}, []string{"cid1", "cid2", "cid3", "cid4"})
	fmt.Println(ret)

	time.Sleep(5 * time.Second)

	ret1 := invokeFabricGetFileMetaData("FileStoreContract", "GetFileMeta", "efbacc5c-9405-46d9-966e-51511bfeb7da")
	fmt.Println(ret1)
	...
	// fmt.Println(ret)
	// ret := invokeFabricGetUser("FileStoreContract", "GetUser")
	// fmt.Println(ret)
...
func invokeFabricStoreFile(contractName string, methodName string, fileId string, fileName string, fileSize C.longlong, totalShards C.longlong, dataShards C.longlong, ipfsUrls []string, cids []string) *C.char { 
	// 1. 设置 gRPC 连接
...
func invokeFabricGetFileMetaData(contractName string, methodName string, fileId string) *C.char { 
	// 1. 设置 gRPC 连接
	clientConnection := newGrpcConnection()
	defer clientConnection.Close()
	...
	result, err := contract.EvaluateTransaction(realName, fileId)
	if err != nil {
		log.Fatalf("Failed to submit transaction: %v", err)
	}
...
func invokeFabricFileStoreGetMetaData(contractName string, methodName string, fileId string) *C.char { 
	// 1. 设置 gRPC 连接
	clientConnection := newGrpcConnection()
	defer clientConnection.Close()
	...
	realName := methodName
	fmt.Println("--> Submit Transaction: FileStoreContract:getFileMeta")
	result, err := contract.SubmitTransaction(realName, fileId)
	if err != nil {
		log.Fatalf("Failed to submit transaction: %v", err)
	}
