...
bool EthChain::Deploy(
    ...
    if (contract_name == "Challenge") {
        ...
        bytecode = ReadSolFile(bin);  // 注意添加 0x 前缀
        if (bytecode.substr(0, 2) != "0x") {
            bytecode = "0x" + bytecode;
        }
    }
    ...
    // 读取 ABI 和 bytecode
    std::string abiStr = ReadSolFile(abi);
...
bool EthChain::Call(const std::string contract_type, const std::string contract_name,
    ...
    if (contract_method == "storeFile") {
        ...
        // char* result = invokeChallengeCreateChallenge(go_contract_name, go_method_name, file_hash, nonce, cid,
        // prover); output = std::string(result);
        Document d;
        d.SetObject();
        ...
        std::string params1 = "'" + params[1] + "' '" + params[2] + "' '" + params[3] + "' '" + params[4] + "' '" + params[5] + "'"; 
        dataStoreFile = ExecNode("eth/script/generate_call_submit_proof.js " + abi + " " + params[0], params1);
    }
...
std::string EthChain::GetAccount() { return Config::Instance().eth_account(); }
