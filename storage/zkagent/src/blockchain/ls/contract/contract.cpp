...
void LsContract::Call(const std::string contract_name, const std::string& address, const std::string& contract_method,
    ...
    std::string abi;
    {
        ...
        abi = ReadSolFile(
            "./contracts/compiled/1/" + contract_name + "/" + contract_address + "/" + contract_name + ".abi");
    }
...
std::string LsContract::Deploy(const std::string& contract_name, const std::vector<std::string>& params) {
    ...
    {
        compileSolidity(contract_name, contract_name);
        abi = ReadSolFile("./contracts/compiled/1/" + contract_name + "/" + contract_name + ".abi");
        bin = ReadSolFile("./contracts/compiled/1/" + contract_name + "/" + contract_name + ".bin");
        if (sdk_->config()->crypto.ssl_type == config::kSslTypeSmSsl) {
            bin = ReadSolFile("./contracts/compiled/1/" + contract_name + "/" + contract_name + ".sm.bin");
        }
    }
