...
std::string LsChain::GetAccount() {
    std::shared_ptr<Account> account = AccountManager::Instance().current_account();
    if (account == nullptr) {
        std::cout << "can not find the account" << std::endl;
        return "";
    } else {
        std::cout << "the account is " << account->address << std::endl;
