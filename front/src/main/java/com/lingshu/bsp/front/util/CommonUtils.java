...
import com.lingshu.chain.sdk.client.protocol.response.ChainBlock;
...
import com.lingshu.chain.sdk.crypto.key.P12KeyStore;
import com.lingshu.chain.sdk.crypto.key.PEMKeyStore;
import com.lingshu.chain.sdk.crypto.signature.ECDSASignatureResult;
import com.lingshu.chain.sdk.crypto.signature.SM2SignatureResult;
import com.lingshu.chain.sdk.crypto.signature.SignatureResult;
import com.lingshu.chain.sdk.model.CryptoType;
import com.lingshu.chain.sdk.model.TransactionReceipt;
import com.lingshu.chain.sdk.tx.common.dto.TxResponse;
import com.lingshu.chain.sdk.tx.parser.ITxResultParser;
import com.lingshu.chain.sdk.tx.parser.impl.TxResultParser;
...
public class CommonUtils {
     ...
     * 2021/08/05 signer <=1.4.3, v=27 >=1.5.0, v=0
     * if using rpcsdk, Signature's v default 27, if using java-sdk, SignatureResult's v default 0, and add 27 in RLP encode
    ...
    public static SignatureResult stringToSignatureData(String signatureData, int encryptType) {
        byte[] byteArr = NumberUtil.hexStringToByteArray(signatureData);
        // 从1开始，因为此处signer返回的byteArr第0位是v
...
//    }
    ...
    public static String signatureDataToString(SM2SignatureResult signatureData) {
        ...
        return NumberUtil.toHexString(byteArr, 0, byteArr.length, false);
    }
    ...
    public static Bytes32 hexStrToBytes32(String hexStr) {
        byte[] byteValue = NumberUtil.hexStringToByteArray(hexStr);
    ...
    public static String utf8StringToHex(String utf8String) {
        return NumberUtil.toHexStringNoPrefix(utf8String.getBytes(StandardCharsets.UTF_8));
    }
    ...
    public static void decodeReceipt(TransactionReceipt receipt, CryptoSuite cryptoSuite) {
        // decode receipt
        ITxResultParser decoder = new TxResultParser(cryptoSuite);
        TxResponse txResponse = decoder.parseReceiptStatus(receipt);
        receipt.setMessage(txResponse.getReceiptMessages());
    }
    ...
    public static void processReceiptHexNumber(TransactionReceipt receipt) {
        ...
        receipt.setGasUsed(NumberUtil.toBigInt(gasUsed).toString(10));
        receipt.setBlockNumber(NumberUtil.toBigInt(blockNumber).toString(10));
    }
     ...
     * @param block
     */
    public static void processBlockHexNumber(ChainBlock.Block block) {
        if (block == null) {
            return;
        }
        String blockNum = Optional.ofNullable(block.getNumber()).orElse("0");
        ...
        block.setNumber(NumberUtil.toBigInt(blockNum).toString(10));
        block.setGasLimit(NumberUtil.toBigInt(gasLimit).toString(10));
        block.setGasUsed(NumberUtil.toBigInt(gasUsed).toString(10));
        block.setTimestamp(NumberUtil.toBigInt(timestamp).toString(10));
    }
     ...
     * @param blockHeader
     */
    public static void processBlockHeaderHexNumber(ChainBlock.Block blockHeader) {
        if (blockHeader == null) {
            return;
        }
        ...
        blockHeader.setGasLimit(NumberUtil.toBigInt(gasLimit).toString(10));
        blockHeader.setGasUsed(NumberUtil.toBigInt(gasUsed).toString(10));
        blockHeader.setTimestamp(NumberUtil.toBigInt(timestamp).toString(10));
    }
    ...
    public static void processTransHexNumber(JsonTransactionResponse trans) {
        ...
        String ledgerId = Optional.ofNullable(trans.getLedgerId()).orElse("0");
        trans.setGas(NumberUtil.toBigInt(gas).toString(10));
        trans.setGasPrice(NumberUtil.toBigInt(gasPrice).toString(10));
        trans.setLedgerId(NumberUtil.toBigInt(ledgerId).toString(10));
    }
    ...
    public static String writePrivateKeyPem(String rawPrivateKey, String address, String userName,
        ...
        CryptoKeyPair cryptoKeyPair = cryptoSuite.getKeyPairFactory().genKeyPair(rawPrivateKey);
        PEMKeyStore.storeKeyPairWithPemFormat(cryptoKeyPair.getHexedPrivateKey(), exportedKeyPath, cryptoKeyPair.getCurveName());
    ...
    public static String writePrivateKeyP12(String p12Password, String rawPrivateKey,
        ...
        if (cryptoSuite.cryptoType == CryptoType.SM_TYPE) {
            signatureAlgorithm = "1.2.156.10197.1.501";
        }
        P12KeyStore.storeKeyPairWithP12Format(cryptoKeyPair.getHexedPrivateKey(), p12Password, exportedKeyPath, cryptoKeyPair.getCurveName(), signatureAlgorithm);
