...
########################### properties config ##########################
image_organization=lingshu
image_name="chainfront"
docker_push="no"
latest_tag=latest
new_tag=
# 父镜像 LINGSHU-CHAIN 的版本(默认版本)
lingshuchain_image_tag="v2.0.0"
...
usage() {
    cat << USAGE  >&2
Usage:
    ${__cmd}    [-h] [-t new_tag] [-p] [-i lingshu]
    -t          New tag for image, required. ex: lingshuchain is v2.0.0, then tag is v2.0.0
    -c          LingShuChain docker image tag, default v2.0.0, equal to lingshu/chainfront:v2.0.0.
    -p          Push image to docker hub, default no.
    -i          Default organization, default lingshu.
    -h          Show help info.
USAGE
    exit 1
}
...
docker build -f "${__root}"/docker/lingshuchain/Dockerfile --build-arg LINGSHUCHAIN_IMG_VERSION="${lingshuchain_image_tag}" -t ${image_repository}:${new_tag}  .
