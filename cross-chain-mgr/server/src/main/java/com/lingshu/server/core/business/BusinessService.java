...
import com.lingshu.server.dto.resp.busi.VcVerifyResp;
...
public class BusinessService {
    ...
    public VcVerifyResp vcSignTest(VerifiableCredentialListDto vcl) throws Exception {
        VcVerifyResp resp = new VcVerifyResp();

        // 验证源VC
        VerifiableCredentialListDto originVc = new VerifiableCredentialListDto();
        originVc.setOrigin(vcl.getOrigin());
        originVc.setTarget(vcl.getOrigin());
        try {
            TxHashResp verifyResult = contractService.callVerifyFunc(JsonOrderProcessor.convert(originVc));
            resp.setOriginVerifyResult(Boolean.TRUE);
        } catch (Exception e) {
            resp.setOriginVerifyResult(Boolean.FALSE);
        }

        // 验证目标
        VerifiableCredentialListDto targetVc = new VerifiableCredentialListDto();
        targetVc.setOrigin(vcl.getTarget());
        targetVc.setTarget(vcl.getTarget());
        try {
            TxHashResp verifyResult = contractService.callVerifyFunc(JsonOrderProcessor.convert(targetVc));
            resp.setTargetVerifyResult(Boolean.TRUE);
        } catch (Exception e) {
            resp.setTargetVerifyResult(Boolean.FALSE);
        }
    ...
    private VerifiableCredentialDto dealVc(ResourceDomainInfo resourceDomainInfo, String privateKeyHex,
        ...
        proof
                .setType("EcdsaSecp256k1RecoverySignature2020")
                .setR(r)
                .setS(s)
                .setV(v)
