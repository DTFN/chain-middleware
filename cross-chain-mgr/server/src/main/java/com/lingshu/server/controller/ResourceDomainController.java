...
public class ResourceDomainController {
    ...
    @GetMapping("get")
    public OpenAPIResp<String> getDomainDetails(@RequestParam String resourceName) {
        try {
            String result = resourceDomainService.getDomainDetails(resourceName);
    ...
    // 是否存在
    @GetMapping("exists")
    public OpenAPIResp<Boolean> doesDomainExist(@RequestParam String resourceName) {
        try {
            boolean result = resourceDomainService.doesDomainExist(resourceName);
            return OpenAPIRespBuilder.success(result);
        } catch (Exception e) {
