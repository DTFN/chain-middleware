...
import com.lingshu.server.dto.resp.busi.MultiDidDocResp;
...
//                break;
    ...
    public MultiDidDocResp didDocFromBusi(String did, ContractTypeEnum contractTypeEnum, String contractAddress) throws Exception {
        String contractFunc = "getDIDDetails";
        String fabricContractFunc = "GetDIDDetails";

        MultiDidDocResp multiDidDocResp = new MultiDidDocResp();
        switch (contractTypeEnum) {
            case CHAINMAKER_WASM:
                ...
                String chainMakerResult = businessContract4Wasm.get(contractFunc, paramsMap);
                Assert.isTrue(StringUtils.hasText(chainMakerResult), () -> new ApiException("账户信息不存在"));
                multiDidDocResp.setChainMaker(chainMakerResult);
                break;
            case LINGSHU_SOLIDITY:
                LsBusinessContract2 businessContract = lingshuBusinessContractService.load(contractAddress);
                com.lingshu.chain.sdk.model.TransactionReceipt lingshuTxr = businessContract.getResult(contractFunc, did);
                log.info("lingshuTxr: {}", JSONUtil.toJsonStr(lingshuTxr));
                String outputHex = lingshuTxr.getOutput();
                String lingshuResult = EthDecoderUtil.decodeOneString(outputHex, false);
                Assert.isTrue(StringUtils.hasText(lingshuResult), () -> new ApiException("账户信息不存在"));
                multiDidDocResp.setLingshu(lingshuResult);
                break;
            case ETHEREUM_SOLIDITY:
                BusinessContract2 businessContract2 = ethereumBusinessContractService.load(contractAddress);
                String ethResult = businessContract2.get(contractFunc, did);
                Assert.isTrue(StringUtils.hasText(ethResult), () -> new ApiException("账户信息不存在"));
                multiDidDocResp.setEthereum(ethResult);
                break;
            case BCOS_SOLIDITY:
                ...
                bcosContractRequest.setFuncParams(did);
                OpenAPIResp bcosTxr = bcosService.get(bcosContractRequest);
                log.info("bcosTxr: {}", bcosTxr);
                Object data = bcosTxr.getData();
                Assert.isTrue(data != null && StringUtils.hasText(String.valueOf(data)), () -> new ApiException("账户信息不存在"));
                multiDidDocResp.setBcos(String.valueOf(data));
                break;
            case BSC_SOLIDITY:
                // 不再使用bsc链
                break;
            case FABRIC_WASM:
                ...
                fabricChainCodeInvokeReq.setFunction(fabricContractFunc);
                ...
                fabricChainCodeInvokeReq.setArgs(Arrays.asList(did));

                FabricChainCodeInvokeResponse fabricTxr = fabricService.invoke(fabricChainCodeInvokeReq);
                Assert.isTrue(fabricTxr.getPayload() != null && StringUtils.hasText(String.valueOf(fabricTxr.getPayload())), () -> new ApiException("账户信息不存在"));
                multiDidDocResp.setFabric(String.valueOf(fabricTxr.getPayload()));
                break;
            default:
                throw new ApiException("不支持的链类型: " + contractTypeEnum);
        }
        return multiDidDocResp;
    }
