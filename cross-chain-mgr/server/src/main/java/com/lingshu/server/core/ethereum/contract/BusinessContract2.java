package com.lingshu.server.core.ethereum.contract;
...
//    ;
    ...
    // 定义DEBUG事件，与Solidity合约中的事件对应
    public static final Event DEBUG_EVENT = new Event(
            "DEBUG",  // 事件名称，必须与合约中定义的完全一致
            Arrays.<TypeReference<?>>asList(
                    new TypeReference<Utf8String>(false) {}  // 对应合约中的string类型参数，非indexed
            )
    );
...
//    }
    ...
    public RemoteFunctionCall<TransactionReceipt> setDidAddress(String methodName, String paramValue0) {
        final Function function = new Function(
                methodName,
                Arrays.<Type>asList(new Address(paramValue0)),
                Collections.<TypeReference<?>>emptyList());
    ...
    public String erc20GetBalance(String businessAddress) throws IOException {
        final Function function = new Function("erc20GetBalance",
                Arrays.<Type>asList(new Utf8String(businessAddress)),
                Arrays.asList(new TypeReference<Utf8String>() {
                })
        );
    ...
    public String erc20GetBalanceVcs(String businessAddress) throws IOException {
        final Function function = new Function("erc20GetBalanceVcs",
                Arrays.<Type>asList(new Utf8String(businessAddress)),
                Arrays.asList(new TypeReference<Utf8String>() {
                })
        );
    ...
    public static List<CrossSaveEth.TestEthEventResponse> getTestEthEvents(TransactionReceipt transactionReceipt) {
        List<Contract.EventValuesWithLog> valueList = staticExtractEventParametersWithLog(DEBUG_EVENT, transactionReceipt);
        ArrayList<CrossSaveEth.TestEthEventResponse> responses = new ArrayList<CrossSaveEth.TestEthEventResponse>(valueList.size());
        for (Contract.EventValuesWithLog eventValues : valueList) {
            CrossSaveEth.TestEthEventResponse typedResponse = new CrossSaveEth.TestEthEventResponse();
