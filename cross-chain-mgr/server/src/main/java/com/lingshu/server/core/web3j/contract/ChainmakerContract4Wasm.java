 ...
 */
@Slf4j
public abstract class ChainmakerContract4Wasm {
    ...
    public ChainmakerContract4Wasm(String contractName, ChainmakerChainClient chainmakerChainClient, ChainmakerAccountUtil4Busi chainmakerAccountUtil) {
    ...
    protected static DeployInfo deployContract(ChainmakerChainClient chainmakerChainClient, String contractName,
        ...
        Request.Payload payload = chainClient.createContractCreatePayload(contractName, version, byteCode, ContractOuterClass.RuntimeType.WASMER, paramsMap);
...
//        String resultStr = byteString.toStringUtf8();
...
//            String message = ChainResultParser.parseResult(messageBytes, String.class);
            String message = "";
            if (messageBytes.size() >= 64 * 2 + 8) {
                messageBytes = messageBytes.substring(64 * 2 + 8);
                message = new String(Numeric.hexStringToByteArray(messageBytes.toStringUtf8()));
            } else {
//                message = HexUtil.decodeHexStr(contractResult.getMessage());
                message = new String(Numeric.hexStringToByteArray(messageBytes.toStringUtf8()));
            }
    ...
    protected TransactionReceipt executeRemoteCallTransaction(String method, Map<String, byte[]> paramsMap) {
        //String methodDataStr = FunctionEncoder.encode(function);
//        String methodDataStr = "1234567";
//        ImmutableMap<String, byte[]> paramsMap = ImmutableMap.of(
//                "content", methodDataStr.getBytes()
//        );
//        String methodDataStr = "1234567";
//        ImmutableMap<String, byte[]> paramsMap = ImmutableMap.of(
//                paramsName, paramsValue.getBytes()
//        );
//        Map<String, byte[]> paramsMap = new HashMap<>();
//        paramsMap.put(paramsName, paramsValue.getBytes());
//        paramsMap.put("didDoc", "12344".getBytes());
        // String method = "find_by_file_hash"; //methodDataStr.substring(0, 10);
        ResultOuterClass.TxResponse txResponse = null;
        try {
            ChainClient chainClient = chainmakerChainClient.getChainClient();
            String txId = Utils.generateTxId();
            ...
            ChainmakerTransaction.TransactionInfo transactionInfo = chainClient
                    .getTxByTxId(txId, ChainmakerChainClient.REQUEST_TIMEOUT);
        ...
        ResultOuterClass.ContractResult contractResult = txResponse.getContractResult();
        if (contractResult.getCode() != 0) {
            throw new RuntimeException("Invoker Contract Failed, code = " + contractResult.getCode() + ", message: " + contractResult.getMessage());
        }
        ...
        TransactionReceipt transactionReceipt = new TransactionReceiptExt();
        transactionReceipt.setTransactionHash(txId);
        // 获取调用合约的账户地址
        transactionReceipt.setFrom(address);
        transactionReceipt.setTo(contractName);
    ...
    protected <T> T executeRemoteCallSingleValueReturn(String method, Map<String, byte[]> paramsMap, Class<T> tClass) {
//        String methodDataStr = FunctionEncoder.encode(function);
//        ImmutableMap<String, byte[]> paramsMap = ImmutableMap.of(
//                paramsName, paramsValue.getBytes()
//        );
//        String method = methodDataStr.substring(0, 10);
        ...
        try {
            ChainClient chainClient = chainmakerChainClient.getChainClient();
            txResponse = chainClient.queryContract(contractName, method, null, paramsMap,
                    ChainmakerChainClient.REQUEST_TIMEOUT);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        if (txResponse.getCode() != ResultOuterClass.TxStatusCode.SUCCESS) {
