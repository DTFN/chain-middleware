...
public class AccountController {
    ...
    @ApiOperation("给DID文档增加ed25519的验证方法")
    @PostMapping("addEd25519")
    public OpenAPIResp<String> addEd25519(@Valid @RequestBody AccountAddEd25519Request request) {
        try {
            String didStr = accountService.addEd25519(request);
            createAccountMetric.incrementSuccess("update");
            return OpenAPIRespBuilder.success(didStr);
        } catch (Exception e) {
            log.error("update error:{}", e.getMessage(), e);
            createAccountMetric.incrementFail("update");
            return OpenAPIRespBuilder.failure(ApiErrorCode.FAILED, e.getMessage());
