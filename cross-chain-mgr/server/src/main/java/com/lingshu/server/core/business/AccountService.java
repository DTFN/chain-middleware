...
public class AccountService {
    ...
    public String addEd25519(AccountAddEd25519Request request) throws Exception {
        // 获取原始DID
        String did = request.getDid();
        String didDetails = didManagerService.getDIDDetails(did);
        DidDto didDocBean = JSONUtil.toBean(didDetails, DidDto.class);

        // 剔除已有的ed25519签名
        List<DidDto.VerificationMethod> verificationMethodNotEd25519 = didDocBean.getVerificationMethod()
                .stream()
                .filter(item -> !"Ed25519VerificationKey2018".equals(item.getType()))
                .collect(Collectors.toList());
        verificationMethodNotEd25519 = new ArrayList<>(verificationMethodNotEd25519);


        // 添加ed25519的验签方法
        String ed25519Func = "#key" + (verificationMethodNotEd25519.size() + 1);
        String ed25519Id = did + ed25519Func;
        String publicKeyHex = Ed25519Signer.derivePublicKey(request.getPrivateKeyHex());
        verificationMethodNotEd25519.add(
                new DidDto.VerificationMethod()
                        .setId(ed25519Id)
                        .setType("Ed25519VerificationKey2018")
                        .setController(did)
                        .setPublicKeyHex(publicKeyHex)
        );
        didDocBean.setVerificationMethod(verificationMethodNotEd25519);

        // 修改 assertionMethod
        didDocBean.setAssertionMethod(
                verificationMethodNotEd25519.stream()
                        .map(DidDto.VerificationMethod::getId)
                        .collect(Collectors.toList())
        );

        // 序列化
        String didDocStr = DIDProcessor.didToString(didDocBean);

        // 更新DID文档
        DIDCreateRequest didUpdateRequest = new DIDCreateRequest();
        didUpdateRequest.setDid(request.getDid());
        didUpdateRequest.setDidDocument(didDocStr);
        TransactionReceipt receipt = didManagerService.updateDID(didUpdateRequest);
    ...
    public DidDocumentResp getDidDocument(String did) throws Exception {
        ...
        Assert.notNull(didDetails, () -> new ApiException("账户信息不存在"));
