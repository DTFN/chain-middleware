...
public class ResourceDomainService {
    ...
    public String getDomainDetails(String resourceName) throws Exception {
        // 首先尝试从缓存获取
        String details = cache.get(resourceName);
        if (details != null) {
            log.info("从缓存获取链上资源域名详情: {}", resourceName);
            return details;
        }

        Boolean exist = resourceDomain.doesDomainExist(resourceName);
        Assert.isTrue(exist, () -> new ApiException(("链上不存在: " + resourceName)));

        // 缓存中没有则从链上获取
        details = resourceDomain.getDomainDetails(resourceName);
        ...
        // 将结果存入缓存
        if (StringUtils.isNotBlank(details)) {
            cache.put(resourceName, details);
        }
        return details;
    }
