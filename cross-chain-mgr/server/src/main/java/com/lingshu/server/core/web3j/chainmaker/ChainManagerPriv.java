...
public class ChainManagerPriv {

    private static final Logger logger = LoggerFactory.getLogger(ChainManagerPriv.class);

    // 使用jdk自带的tls
    static String OPENSSL_PROVIDER = "JDK";
    static String TLS_NEGOTIATION = "TLS";

    // chains' map
    private Map<String, ChainClient> chains = new HashMap<>();
    // for singleton mode
    private static ChainManagerPriv chainManager = new ChainManagerPriv();

    private ChainManagerPriv() {
    }

    // singleton getInstance
    public static ChainManagerPriv getInstance() {
        return chainManager;
    }
