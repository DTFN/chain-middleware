...
public class DIDManagerController {
    ...
    @ApiOperation("查询业务链上的DID信息")
    @GetMapping("did-doc-from-busi")
    public OpenAPIResp<MultiDidDocResp> didDocFromBusi(
            @RequestParam String did,
            @RequestParam String contractType,
            @RequestParam String contractAddress
    ) {
        try {
            MultiDidDocResp resp = contractService.didDocFromBusi(did, ContractTypeEnum.getByCode(contractType), contractAddress);
            return OpenAPIRespBuilder.success(resp);
        } catch (Exception e) {
