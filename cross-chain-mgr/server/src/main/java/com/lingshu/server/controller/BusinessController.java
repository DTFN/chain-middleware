...
public class BusinessController {
    ...
    @ApiOperation("验证链账户VC签名")
    @PostMapping("vcSignTest")
    public OpenAPIResp<VcVerifyResp> vcSignTest(@Valid @RequestBody VerifiableCredentialListDto request) {
        try {
            return OpenAPIRespBuilder.success(businessService.vcSignTest(request));
        } catch (Exception e) {
            log.error("call error:{}", e.getMessage(), e);
            return OpenAPIRespBuilder.failure(ApiErrorCode.FAILED, e.getMessage());
        }
