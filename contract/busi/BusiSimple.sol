...
contract BusiSimple {
    ...
    // 构造方法
    function BusiSimple(address didManagerAddressExt) public {
        didManagerAddress = didManagerAddressExt;
    }
    ...
    function hexStringToBytes32(string memory hexString) public returns (bytes32) {
        ...
        if (bytesString.length == 42) {
            return hexStringToBytes32(JsmnSolLib.getBytes(hexString, 2, 42));
        }
        if (bytesString.length == 40) {
            return hexStringToBytes32(string(abi.encodePacked("000000000000000000000000", hexString)));
        }
        require(bytesString.length == 64, string(abi.encodePacked("Invalid hex string length ", uintToString(bytes(hexString).length), ", address is ", hexString)));
