...
contract Busi {
    ...
    // 构造方法
    function Busi(address didManagerAddressExt) public {
        didManagerAddress = didManagerAddressExt;
    }
    ...
    // 下面是示例
    function run(string json) external {
        // 1 提取两个VC
    ...
    function verifyVc(string memory vcJson) public {
        ...
        bytes32 rBytes = hexString64ToBytes32(valueByKey(proof, "r"));
        bytes32 sBytes = hexString64ToBytes32(s);
        ...
        bytes32 contentHashBytes = getEthSignedMessageHash(vcStrNoProof);

        // 4 查询did合约
        string memory verifyAddress = parseDidAndDidMerhodId(proof);

        // 5 验签（验证地址是否一致） test:address 0xbd5d9f6b1b70cfd36c90aa11a8830a43062ab4ea
        address calAddress = ecrecover(contentHashBytes, vuint, rBytes, sBytes);
        require(
            hexString40ToBytes32(verifyAddress)
            ==
    ...
    function hexString64ToBytes32(string memory hexString) public returns (bytes32) {
        if (bytes(hexString).length == 66) {
            return hexString64ToBytes32(JsmnSolLib.getBytes(hexString, 2, 66));
        }
        require(bytes(hexString).length == 64, string(abi.encodePacked("Invalid hex string length ", uintToString(bytes(hexString).length), ", address is ", hexString)));
    ...
    function hexString40ToBytes32(string memory hexString) public returns (bytes32) {
        if (bytes(hexString).length == 42) {
            return hexString40ToBytes32(JsmnSolLib.getBytes(hexString, 2, 42));
        }
        require(bytes(hexString).length == 40, string(abi.encodePacked("Invalid hex string length ", uintToString(bytes(hexString).length), ", address is ", hexString)));
        ...
        for (uint256 i = 0; i < bytesLength; i += 2) {
            ...
            // 转换字符到对应的数值
            uint8 byteValue = (hexCharToByte(b1) << 4) | hexCharToByte(b2);
            result |= bytes32(uint256(byteValue) << (248 - (12 * 8) - (i / 2) * 8));
        }
    ...
    function test_call_did(string memory key) public {
        DIDManagerShadow didManager = DIDManagerShadow(didManagerAddress);
        string memory didDoc = didManager.getDIDDetails(key);
        emit DEBUG(didDoc);
    }
    ...
    function test_call_did_and_parse(string memory did, string memory methodId) public returns (string) {
        return findAddressByMethodIdFromDidManager(did, methodId);
    }
        ...
        for (uint i = 0; i < methodNumber; i++) {
            ...
            if (leftPosition > 0 && rightPosition > 0) {
                ...
                if (JsmnSolLib.strCompare(id, methodId) == 0) {
                    return valueByKey(valueStr, "ethereumAddress");
                }
    ...
    function findCharPosition(string memory str, bytes1 charIn, uint startPosition) public pure returns (int) {
        bytes memory strBytes = bytes(str);

        for (uint i = startPosition; i < strBytes.length; i++) {
            if (strBytes[i] == charIn) {
    ...
    function findCharNumber(string memory str, bytes1 charIn, uint startPosition) public pure returns (uint) {
        bytes memory strBytes = bytes(str);
        uint number = 0;
