...
pub extern "C" fn erc20TransferVcs() {
    ...
    if from_old_amount < amount {
        ctx.error(
            format!(
                "from user not have enough, from_old_amount = {}, amount = {}",
                from_old_amount, amount
            )
            .as_str(),
        );
        return;
